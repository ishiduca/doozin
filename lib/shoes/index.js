...
module.exports = shoe(function (shoes) {
    var errs = through.obj(function (err, enc, done) {
        console.error('[errs.emit("error", %s)]', String(err))
        done(null, Error.prototype.toString.apply(err))
    })
    ...
    errs.pipe(es.stringify()).pipe(shoes)
